...
const resolveLocalModule = (specifier, importerDir) => {
    ...
    if (!resolved) throw new Error(`Unable to resolve "${specifier}" from "${importerDir}"`);
    return resolved;
};
...
const loadExamples = () => {
    const mod = loadTsModule(path.join(rootDir, 'src/examples/index.ts'));
...
const buildEntries = (examples) => {
    ...
    return examples.map((example) => {
        ...
        return { radical, filename, url: `${renderUrl}${radical}` };
    });
};
...
const cacheMissingEntry = async (entry) => {
    ...
    try {
        ...
        if (!svg.includes('<svg')) {
            console.warn(`[examples:cache] Unexpected response for ${entry.url}`);
            return false;
        }
...
const main = async () => {
    console.log(`[examples:cache] Render URL: ${renderUrl}`);
